...
double softplus_revert (double x)
{
    return log(exp(x)-1);
}
...
box get_yolo_box(float *x, float *biases, int n, int index, int i, int j, int lw, int lh, int w, int h, int stride)
{
    ...
    b.w = softplus(x[index + 2*stride]) * biases[2*n]   / w;
    b.h = softplus(x[index + 3*stride]) * biases[2*n+1] / h;
    return b;
}
...
ious delta_yolo_box(box truth, float *x, float *biases, int n, int index, int i, int j, int lw, int lh, int w, int h, float *delta, float scale, int stride, float iou_normalizer, IOU_LOSS iou_loss)
{
    ...
    if (iou_loss == MSE)    // old loss
    {
        ...
        float tw = softplus_revert(truth.w*w / biases[2 * n]);
        float th = softplus_revert(truth.h*h / biases[2 * n + 1]);
